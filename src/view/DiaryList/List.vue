<template>
    <div class="diary-list-group-container" :style="`min-height: ${projectStore.insets.heightPanel}px`">
        <transition enter-active-class="animated-fast slideInDown" leave-active-class="animated-fast slideOutUp">
            <DiaryFilterPanel
                v-if="projectStore.isShowSearchBar || hasActiveFilters"
                id="keyword"
        ...
        <div v-show="!isLoading && !isHasMore" class="end-of-diary">
            <div class="no-diary-list" v-if="diaryList.length < 1">
                <div class="empty-title">{{ emptyStateTitle }}</div>
                <div class="empty-subtitle">{{ emptyStateSubtitle }}</div>
                <div class="empty-actions">
                    <button v-if="hasActiveFilters" type="button" @click="clearAllFilters">清除筛选</button>
                    <button type="button" @click="createFirstDiary">写一篇</button>
                </div>
            </div>
            <p>
...
import {dateFormatter, dateProcess, EnumWeekDayShort, formatDiaryDateRangeLabel} from "@/utility"
...
import DiaryFilterPanel from "@/view/DiaryList/DiaryFilterPanel.vue"
...
const categoryLabelMap = computed<Record<string, string>>(() => {
    ...
    useStatisticStore().categoryAll.forEach(item => {
        map[item.name_en] = item.name
    })
    return map
})
const emptyStateTitle = computed(() => hasActiveFilters.value ? '没有找到匹配的日记' : '还没有日记')
const emptyStateSubtitle = computed(() => hasActiveFilters.value ? '换个关键词，或清除筛选后再看看。' : '从一句话开始，先把今天留住。')
